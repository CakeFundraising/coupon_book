...
initialize = ->
  ...
  $.getScript "//s7.addthis.com/js/300/addthis_widget.js#async=1", ->
    ...
    window.addthis.toolbox ".addthis_sharing_toolbox, .addthis_native_toolbox, .addthis_jumbo_share"
    window.addthis.init()
    return
  return
...
CakeCouponBook.addThis.share.coupon_book = (fr, url)->
  ...
  window.addthis_share =
    ...
    shorteners: bitly: {}
    templates: twitter: "Support #{fr} & get awesome deal vouchers too! #{url} via @EatsForGood"
  return
